...
$sql = $db->query("SELECT id, replace(JSON_EXTRACT(contenido, '$.nombre'), '\"', '') AS titulo, replace(JSON_EXTRACT(contenido, '$.fotos[0].nombreRuta'), '\"', '') as foto,
	...
	replace(JSON_EXTRACT(contenido, '$.peruanos.adultos'), '\"', '') as precio, replace(JSON_EXTRACT(contenido, '$.duracion'), '\"', '') as duracion, tipo
FROM `tours` 
WHERE activo = 1 and visible = 1
AND JSON_EXTRACT(contenido, '$.departamento') = {$_POST['departamento']}
ORDER BY FIELD(tipo,'{$_POST['tipo']}') asc, FIELD(JSON_EXTRACT(contenido, '$.departamento'), '{$_POST['departamento']}') DESC, RAND() limit 3 "); //and tipo = 1
if($sql ->execute()){
